    ...
    return (
      <div className="min-h-screen bg-navy flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 bg-brand rounded-xl flex items-center justify-center text-white text-2xl font-bold mx-auto mb-3">N</div>
          <p className="text-white text-sm font-medium">ANC Néphro Centre</p>
  ...
  if (profile.status === 'suspended') {
    return (
      <div className="min-h-screen bg-navy flex flex-col items-center justify-center px-8 text-center">
        ...
        <p className="text-blue-300 text-sm mb-6">Veuillez contacter le bureau de l'ANC.</p>
        ...
        <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-sm bg-white border-t border-gray-100 flex pb-safe z-50">
          {NAV.map(n => (
            <button key={n.id} onClick={() => { setTab(n.id); if (n.id !== 'tools') setToolsSub(undefined); }}
              className={`flex-1 flex flex-col items-center py-2 gap-0.5 transition-colors ${tab === n.id ? 'text-brand-dark' : 'text-gray-400'}`}>
              <span className="text-lg leading-none">{n.icon}</span>
              <span className="text-xs font-medium">{n.label}</span>
              {tab === n.id && <span className="w-1 h-1 bg-brand-dark rounded-full"></span>}
            </button>
          ))}
        </nav>
      </div>
