...
interface Member { id:string; full_name:string; specialty:string; hospital:string; phone:string; status:string; role:string; member_since:string; }
...
export default function AdminPanel() {
  ...
  const toggleAdmin=async(id:string,currentRole:string)=>{
    ...
    await supabase.from('profiles').update({role:newRole}).eq('id',id); load();
  };
      ...
      <div className="px-4 pb-8 space-y-3 flex-1">
        ...
        {members.map(m=>(
          <div key={m.id} className="bg-white rounded-2xl border border-gray-100 p-4">
            ...
            {m.status==='suspended'&&(
              <button onClick={()=>updateStatus(m.id,'active')} className="w-full py-2 bg-green-50 border border-green-200 rounded-xl text-xs font-semibold text-green-700 hover:bg-green-100">Réactiver</button>
            )}
          </div>
        ))}
      </div>
